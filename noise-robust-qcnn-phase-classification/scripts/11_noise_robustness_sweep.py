...
def first_failure(frame: pd.DataFrame, floor: float) -> float | None:
    sweep = frame[frame["sweep_kind"] == "two_qubit_depolarizing"].sort_values("depolarizing_2q")
    failed = sweep[sweep["balanced_accuracy"] < floor]
...
def main():
    ...
    summary = {
        ...
        "first_tested_failure_probability": first_failure(frame, floor),
        "grid_is_coarse": True,
    }
