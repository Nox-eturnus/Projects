...
def run_hardware(op_name: str, a: int, b: int) -> None:
    ...
    try:
        ...
        # List every backend the account can see so the user can pick one
        all_backends = service.backends()
        hw_backends = [b for b in all_backends if not b.configuration().simulator]
        if hw_backends:
            print("\n  WARNING: least_busy() found no operational hardware.")
            print("  Available hardware backends on your plan:")
            for b in hw_backends:
                status = b.status()
                print(f"    - {b.name}  ({b.num_qubits}q, "
                      f"operational={status.operational}, "
                      f"pending_jobs={status.pending_jobs})")
            # Try the first hardware backend regardless of status
            backend = hw_backends[0]
            print(f"\n  Falling back to: {backend.name}")
        else:
            print("\n  ERROR: No hardware backends are available on your IBM account.")
            print("  Possible reasons:")
            print("    • The IBM Open (free) plan may have limited/no availability right now.")
            print("    • All backends may be under maintenance.")
            print("    • Your saved credentials may be for a different service instance.")
            print("  Check https://quantum.ibm.com for backend status.")
            return
